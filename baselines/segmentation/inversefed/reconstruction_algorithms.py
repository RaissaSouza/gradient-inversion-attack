...
class GradientReconstructor():
    ...
    def __init__(self, model, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1):
        ...
        print('reconstruction is called')
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        ...
        if labels is None:
            raise ValueError("Segmentation reconstruction requires mask labels.")
            ...
            #         labels = torch.nn.functional.mse_loss(pred, labels)
            #         print(f'Loss: {labels}')
            #         print(f'self.loss_fn: {self.loss_fn}')
            #         return  labels
            #         # torch.mean(torch.sum(- labels * torch.nn.functional.log_softmax(pred, dim=-1), 1))
            ...
            for iteration in range(max_iterations):
                ...
                with torch.no_grad():
                    ...
                    if self.config['boxed']:
                        dm, ds = self.mean_std
                        x_trial.data.clip(dm-1.5*ds,dm+1.5*ds)
                    ...
                    if (iteration + 1) % 500 == 0:
                        if self.config['filter'] == 'none':
                        ...
                        elif self.config['filter'] == 'gauss2':
                            # Apply 2D Gaussian smoothing with a fixed kernel
                            kernel_size = 5
                            sigma = 1.0
                            ...
                            # Reshape into conv2d weight
                            gauss2d = gauss2d.view(1, 1, kernel_size, kernel_size).to(x_trial.device, x_trial.dtype)

                            # Expand to apply per-channel
                            gauss2d = gauss2d.expand(x_trial.size(1), 1, kernel_size, kernel_size)

                            # Apply filter
                            x_trial.data = torch.nn.functional.conv2d(x_trial, gauss2d, padding=kernel_size//2, groups=x_trial.size(1))
                        else:
                            raise ValueError()
    ...
    def _average_trials(self, x, labels, input_data, stats):
        ...
        if self.reconstruct_label:
            ...
            raise RuntimeError("Label reconstruction not supported for segmentation with Dice.")
