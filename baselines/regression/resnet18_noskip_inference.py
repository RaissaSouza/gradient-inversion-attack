...
model.load_state_dict(torch.load("resnet18_noskip_harm_age.pt", map_location=device))
model.eval()
...
results_df.to_csv("inf_resnet18_noskip_regression_results.csv", index=False)

print("Inference completed. Results saved to inf_resnet18_noskip_regression_results.csv")
print(f"MAE:  {mae:.4f}")
